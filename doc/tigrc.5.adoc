...
'diff-highlight' (mixed)::
	...
	value is used as the command to run. The command may include arguments
	when the value is quoted, e.g. `set diff-highlight = "delta
	--color-only"`. ANSI colors and attributes emitted by the program are
	rendered. Text for which the program sets colors is shown with those
	colors, while text with only attributes keeps the colors of the line.
	The program has no access to the terminal, so programs that detect
	the terminal background by querying it fall back to their default.
	True colors are rendered as is on terminals whose terminfo entry
	declares direct colors, such as `xterm-direct` and `tmux-direct`,
	when Tig is built with ncurses 6.1 or later. Elsewhere they are
	approximated by the 256-color palette, and the program is run with
	the `COLORTERM` environment variable cleared so that it picks palette
	colors itself. Regions
	marked with reverse video on added and removed lines, as done by
	'diff-highlight', are governed by `color diff-add-highlight` and
...
'wrap-lines' (bool)::
	...
	Not compatible with line numbers enabled. Lines containing ANSI
	escape sequences are not wrapped.
...
*colorN-1* (where *N* is the number of colors supported by your terminal). On terminals with direct color support, *color8* and above
are rendered with the colors of the xterm palette rather than with those
