...
'diff-highlight' (mixed)::
	...
	value is used as the path. ANSI colors and attributes emitted by the
	program are rendered, with true colors approximated by the 256-color
	palette. Regions marked with reverse video on added and removed lines,
	as done by 'diff-highlight', are governed by `color diff-add-highlight`
	and `color diff-del-highlight`.
