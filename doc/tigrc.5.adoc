...
'side-by-side' (bool)::
	...
	Show diff chunks in the diff view side by side, with the old version
	of the file on the left and the new version on the right. Runs of
	removed and added lines are paired row by row, and each side is
	truncated to half of the width. File and chunk headers span both
	sides and are set apart by horizontal rules. Horizontal scrolling
	moves both sides and 'wrap-lines' does not apply to side-by-side
	rows. With the `file` option of the line-number column each side
	shows the line numbers of its file. Searches match each side of a
	row separately, while the
	`%(text)` variable holds both sides separated by a newline, and
	blaming a row with both sides uses its old side. Combined diffs,
	word diffs and the stage view always use the unified layout. Off by
	default. The separator and rules are colored with
	`color side-by-side`.
...
line-number::
	...
	- 'file' (bool): Whether to show the line numbers of the file being
	  diffed instead of those of the view, in the diff and stage views.
	  Removed lines get their number in the old file, other lines in a
	  diff chunk their number in the new file and lines outside of diff
	  chunks none.
...
|side-by-side		|The separator and rules of the side-by-side diff layout.
|id			|The commit ID.
